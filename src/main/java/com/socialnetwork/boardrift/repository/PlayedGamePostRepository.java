...
public interface PlayedGamePostRepository extends JpaRepository<PlayedGamePostEntity, Long> {
    ...
    List<PlayedGamePostEntity> findAllByPostCreatorOrFriends(@Param("postCreator") UserEntity userEntity,
                                                             Pageable pageable);

    List<PlayedGamePostEntity> findByBasePostPostCreatorId(Long userId, Pageable pageable);

    @Query("SELECT pgp FROM PlayedGamePostEntity pgp" +
            " WHERE SIZE(pgp.basePost.reports) > 0" +
            " ORDER BY SIZE(pgp.basePost.reports) DESC")
